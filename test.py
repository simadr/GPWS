...
def create_testMode2(tcri, tcrf, ralti, raltf, nb_points, filename):
    x=0
    y=0
    z=0
    fpa = -3*math.pi/180
...
def create_testMode3(phase, vzi, vzf, ralti, raltf, nb_points, filename):
    x=0
    y=0
    z=0
    fpa = -3*math.pi/180
    ...
    ralt = ralti
    fic = open(filename, "w")
    fmstogpws = "FMS_TO_GPWS Phase={0} DA={1} DH={2}\n".format(phase,0, 0)
    fic.write(fmstogpws)
    for i in range(nb_points):
        time = "Time t={}\n".format(i)
        radio_alt =  "RadioAltimeter groundAlt={}\n".format(ralt)

...
def mode1(Alt,Vz):
    if enveloppePullUp(Vz) >= Alt:
        return("PullUp")
    elif enveloppeSinkRate(Vz) >= Alt:
        return("SinkRate")


def enveloppe1PullUp(x):
    if x <= 1750:
        return 0
    elif x <= 3000:
...
def enveloppe2TerrainTerrain(x):
    if x <= 2277:
        return 0
    elif x <= 3000:
